<script lang="ts">
	...
	import { manualPartSchema, type OnshapePartSchema } from '$lib/zod-schemas';
	...
	interface Props {
		onshapePartForm: SuperValidated<Infer<OnshapePartSchema>>;
	...
	const form = superForm(onshapePartForm, {
...
<Dialog.Content class="sm:max-w-xl" showCloseButton={false}>
	{#if !isLoadingData}
		<form method="POST" action="?/onshapePart" use:enhance>
			...
			<Dialog.Footer>
				<Dialog.Close type="reset" class={buttonVariants({ variant: 'outline' })}
					>Cancel</Dialog.Close
				>
